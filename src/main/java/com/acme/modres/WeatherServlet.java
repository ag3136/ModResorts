...
public class WeatherServlet extends HttpServlet {
  ...
  private static final String WEATHER_API_SECRET_NAME_ENV = "WEATHER_API_KEY_SECRET_NAME";
  ...
  private static String mockKey(String toBeMocked) {
    ...
    String lastToKeep = toBeMocked.substring(Math.max(0, toBeMocked.length() - 3));
    return "*********" + lastToKeep;
  }

  private String getWeatherApiKeyFromKeyVault() {
    String secretName = System.getenv(WEATHER_API_SECRET_NAME_ENV);
    if (secretName != null && secretName.trim().length() > 0) {
      try {
        return new AzureKeyVaultSecretProvider().getSecret(secretName);
      } catch (RuntimeException e) {
        logger.log(Level.WARNING, "Unable to read weather API key from Azure Key Vault", e);
      }
    }

    // Optional fallback for local development only; cloud deployments should use Azure Key Vault.
